...
def main() -> None:
    args = parse_args()
    if not args.user_file and not args.raw_file and not args.processed_file:
        raise SystemExit("Provide at least one of --user-file, --raw-file, or --processed-file.")
    ...
    with SessionLocal() as session:
        ...
        if args.user_file:
            snapshot = import_garmin_user_file(session, user.id, args.user_file)
            results["user_snapshot_id"] = str(snapshot.id)
        if args.raw_file:
