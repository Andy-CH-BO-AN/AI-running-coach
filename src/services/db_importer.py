...
def import_garmin_raw_file(session: Session, user_id, path: str | Path) -> dict[str, int]:
    ...
    for activity_data in activities:
        ...
        # A partial strength retry must never erase a previously persisted complete
        # activity. First-seen partial activities are still inserted; once a full
        # Garmin payload arrives it follows the normal upsert path.
