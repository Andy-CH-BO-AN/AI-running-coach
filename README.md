...
| `src/pipeline/runner.py` | 主流程 orchestration：載入活動 payload、preprocessing、deterministic context 與 Gemini 分析。 |
...
| `src/db/` | SQLAlchemy 2.0 models、DB settings/session、repository layer 與 local/cloud sync。 |
...
DB settings、DB sync target engine 與 coach context zone builder 都已從大型
orchestration 檔案中拆出。公開入口與 JSON/DB 行為維持相容，舊測試仍保留。

下一階段建議按小批次繼續拆，不要一次重構整個 repository：

1. `src/preprocessing/coach_context.py` session builder：抽出 raw lookup、
   session type、training load/effect、segment/environment builder。
2. `src/preprocessing/coach_context.py` athlete metrics：抽出 physio seed、
   pace zones、running mechanics 與 cross-training summary。
3. `src/preprocessing/coach_context.py` weekly/report enforcement：分離 weekly
   aggregation、next-week seed、evidence facts 與 AI report deterministic overlay。
4. `src/pipeline/runner.py` payload provider：在 artifact/import service 穩定後，
   把 DB-backed load/fetch/fallback policy 移到 `src/pipeline/activity_payloads.py`。
5. DB sync internals：在 facade 穩定前提下，再分離 table planning、row copy 與
   snapshot compare；`sync_database_targets()` / `compare_database_targets()` 保持公開 API。
