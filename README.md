...
| `src/db/` | SQLAlchemy 2.0 models、DB settings/session、repository layer；`sync.py` 是 local/cloud sync facade，copy/conflict 與 snapshot/compare internals 分別在 `sync_copy.py`、`sync_compare.py`。 |
...
builder 與 runner payload provider 都已從大型 orchestration 檔案中拆出。
公開入口與 JSON/DB 行為維持相容，舊測試仍保留。

下一階段建議按小批次繼續拆，不要一次重構整個 repository：

1. DB mapper/repository boundary：讓 repositories 專注 SQL persistence，mapper
   helper 作為明確 mapping API。
2. Test DB guard：集中 `TEST_DATABASE_URL` safety validation，保留拒絕主 DB 與
   非 test DB 的防呆。
