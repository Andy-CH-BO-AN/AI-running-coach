...
- 可選的 Phase 2 PostgreSQL persistence layer：保留 Garmin raw JSON、user profile snapshots、activity splits、swimming lengths、feature versions、weekly summaries，以及 AI report metadata。
...
- `src/scripts/import_garmin_files.py`: Phase 2 DB import CLI。
...
請在本機 `.env` 填入 `GARMIN_PASSWORD`、`GEMINI_KEY` 與 `POSTGRES_PASSWORD`。`POSTGRES_PASSWORD` 不放進 git tracked files，避免 secret scanner 將範例密碼當成憑證。
