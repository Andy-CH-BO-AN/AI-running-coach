...
DB settings、DB sync target engine、coach context zone builder 與 runner
payload provider 都已從大型 orchestration 檔案中拆出。公開入口與 JSON/DB
行為維持相容，舊測試仍保留。

下一階段建議按小批次繼續拆，不要一次重構整個 repository：

1. DB sync internals：在 facade 穩定前提下，再分離 table planning、row copy 與
   snapshot compare；`sync_database_targets()` / `compare_database_targets()` 保持公開 API。
2. DB mapper/repository boundary：讓 repositories 專注 SQL persistence，mapper
   helper 作為明確 mapping API。
3. Test DB guard：集中 `TEST_DATABASE_URL` safety validation，保留拒絕主 DB 與
   非 test DB 的防呆。
