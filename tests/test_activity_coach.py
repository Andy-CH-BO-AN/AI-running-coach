...
def test_activity_coach_rejects_zero_load_claim_when_load_is_unknown(monkeypatch: pytest.MonkeyPatch) -> None:
    analysis = "Garmin 訓練負荷目前不可得，但本次為 0 TSS 且負荷偏低，建議增加 10 TSS。" * 4
    monkeypatch.setattr(activity_coach.coach_agent, "MODEL_FALLBACKS", ("provider",))
    monkeypatch.setattr(
        activity_coach.coach_agent,
        "_generate_content_with_retries",
    ...
    with pytest.raises(activity_coach.ActivityCoachError, match="unknown training load"):
        activity_coach.generate_activity_report(
            _spec(input_json={"activity": {"activity_id": 123, "training_load": None}})
        )


def test_activity_coach_allows_measured_zero_when_historical_load_is_unknown(monkeypatch: pytest.MonkeyPatch) -> None:
    analysis = "本次 Garmin 明確記錄 0 TSS；此數值是本活動的實測結果，不受歷史週資料缺漏影響。" * 4
    monkeypatch.setattr(activity_coach.coach_agent, "MODEL_FALLBACKS", ("provider",))
    monkeypatch.setattr(
        activity_coach.coach_agent,
        "_generate_content_with_retries",
    ...
    report = activity_coach.generate_activity_report(
        _spec(input_json={
            "activity": {"activity_id": 123, "training_load": 0},
            "recent_training_weeks": [{"derived_training_load": None}],
        })
    )
...
def test_activity_coach_allows_neutral_unknown_load_copy(monkeypatch: pytest.MonkeyPatch) -> None:
    analysis = "本次 Garmin 負荷資料不足，因此不判斷負荷高低；改依已知時長與內容保守安排恢復。" * 4
    monkeypatch.setattr(activity_coach.coach_agent, "MODEL_FALLBACKS", ("provider",))
    monkeypatch.setattr(
        activity_coach.coach_agent,
        "_generate_content_with_retries",
