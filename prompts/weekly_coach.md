...
3. 每堂關鍵課的 `description` 都要說明它對 `core_goal` 的作用。資料不足時採保守安排並說明，不要捏造 Garmin 數據。
   `sessions[].training_load`、週總負荷、Training Effect、心率及其他 metrics 都可能是 `null`；`null` 表示 Garmin 資料不可得，絕不是實測 0。任一納入活動的負荷未知時，週總負荷也未知，不得引用已知活動的小計、描述為 0 TSS／負荷偏低，或據此建議增加 TSS。只有 deterministic facts 明確提供 0 時才可描述實測 0。
...
- `strength`、每個 `strength.sets[].reps` 或週級肌力 aggregate 的組數、次數、容量為 `null` 表示不可得，不是 0；不得補算或據此下結論。
